...
export default function LoginConfig({ email }: LoginProps) {
  const [state, formAction, isPending] = useActionState(checkUser, null);
    ...
    <>
        ...
        <section className={styles.box_msgWp}>
          <div className={styles.box_msg}>
            ...
            <div className={styles.icon_container}>
              <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="#9b2424" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M18 6 6 18" /><path d="m6 6 12 12" />
              </svg>
            </div>
            <div className={styles.message_text_container}>
              <p className={styles.message_text}>Error</p>
              <p className={styles.sub_text}>{state.error}</p>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15" strokeWidth="0" fill="none" stroke="currentColor" className={styles.cross_icon}>
      ...
      <main className={`${styles.mainLogin} w100 d_flexComplet p_relative`}>
        ...
        <div className={`heroWrapper p_relative d_flex ${styles.heroWrapperLogIn}`}>
          <Image width={700} height={700} className={`${styles.hero} p_absolute`} src="/images/svgLogin.svg" alt="New ideas" loading='eager' />
        </div>
        ...
        <form className={styles.contentWrapperLogIn} action={formAction}>
          ...
          <div className={`${styles.logIn_wrapper} p_relative d_flex`}>
            <div className={`${styles.logIn} ${styles.user} p_relative`}>
              <label htmlFor="user" className="sr-only">E-mail</label>
              <input
                id="user"
                type="email"
                placeholder="E-mail"
                ...
                disabled={isPending}
              />
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#212b46" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="p_absolute">
                <circle cx="12" cy="8" r="5" /><path d="M20 21a8 8 0 0 0-16 0" />
              </svg>
            </div>

            <div className={`${styles.logIn} ${styles.password} p_relative`}>
              <BtnEyePwComponent pageStyle={styles.pwEye} disabled={isPending} />
            </div>
          </div>
          ...
          <div className={`${styles.action} d_flexComplet`}>
            ...
            <button
              type="submit"
              className={`${styles.logIn} logIn txtWhite bgBlue d_inlineBlock`}
              name="log-in"
              disabled={isPending}
              aria-busy={isPending}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="p_relative">
